...
class InternalSlider: UISlider {
    ...
    var roundedValue: Float {
        get {
            return roundValue(super.value, to: increment)
        }
    }
    ...
    @objc private func endSliding() {
        setValue(roundValue(super.value, to: increment), animated: true)
        sendActions(for: .valueChanged)
    }
    
    func roundValue(_ value: Float, to increment: Float) -> Float {
        if increment == 0 {
            return value
        }
        return increment * Float(Darwin.round(value / increment))
    }
}
